...
> **项目状态：v0.12.0 预览版。** IntentRoute AI 适合测试与早期采用，尚未经过大规模生产验证。AI 输出可能不完整或有误；每条生成的规则都经过本地校验、以禁用状态写入，必须由你显式启用。

v0.12.0 新增批量规则管理：扩展多选、单事务批量启用/禁用/删除。每个发布包仍内嵌 v0.9.0 引入的未签名 `provenance.json` 构建溯源清单；代码签名本身仍需要证书。
...
1. 从 [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases) 下载 `IntentRoute-AI-v0.12.0-win-x64.zip` 与 `.sha256` 文件。
2. 校验校验和。
