...
> **项目状态：v0.18.0 预览版。** IntentRoute AI 适合测试与早期采用，尚未经过大规模生产验证。AI 输出可能不完整或有误；每条生成的规则都经过本地校验、以禁用状态写入，必须由你显式启用。

v0.18.0 完成界面翻新：每个页面都有自己的主题色页头横幅（规则/AI/设置蓝色、策略体检绿色、推演/进程琥珀色、日志/关于中性色），运行日志页与进程列表页统一为「页头 + 工具栏卡片 + 内容卡片」结构，日志以等宽终端风格渲染。此前 v0.16 将全部界面统一到一套深色设计系统与分组侧栏，v0.17 对表格、KPI 卡与对话框做了组件库打磨；v0.15.0 加入按完整规则身份去重的规则导入预览。每个发布包仍内嵌 v0.9.0 引入的未签名 `provenance.json` 构建溯源清单；代码签名本身仍需要证书。
...
1. 从 [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases) 下载 `IntentRoute-AI-v0.18.0-win-x64.zip` 与 `.sha256` 文件。
2. 校验校验和。
