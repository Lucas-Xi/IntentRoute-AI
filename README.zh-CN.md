...
> **项目状态：v0.17.0 预览版。** IntentRoute AI 适合测试与早期采用，尚未经过大规模生产验证。AI 输出可能不完整或有误；每条生成的规则都经过本地校验、以禁用状态写入，必须由你显式启用。

v0.17.0 是组件库打磨版：全部表格共用容器化处理（深色表头带、整行悬停、行分隔线）、策略体检 KPI 卡三层结构（色点+标签 / 24px 数值 / 本地化脚注）、规则页空态直接可操作（添加规则/导入双按钮）、对话框 200ms 缩放淡入进场——设计模式借鉴自 Shadcnblocks/ReUI/Motion 组件档案并映射到本应用自己的设计令牌。v0.16.0 把主窗口与对话框统一到同一套深色设计系统，带分组侧栏与重设计外壳；v0.15.0 加入规则导入预览，按完整规则身份做重复检测。每个发布包仍内嵌 v0.9.0 引入的未签名 `provenance.json` 构建溯源清单；代码签名本身仍需要证书。
...
1. 从 [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases) 下载 `IntentRoute-AI-v0.17.0-win-x64.zip` 与 `.sha256` 文件。
2. 校验校验和。
