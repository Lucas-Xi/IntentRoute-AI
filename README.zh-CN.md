...
> **项目状态：v0.16.0 预览版。** IntentRoute AI 适合测试与早期采用，尚未经过大规模生产验证。AI 输出可能不完整或有误；每条生成的规则都经过本地校验、以禁用状态写入，必须由你显式启用。

v0.16.0 是界面翻新版：主窗口与对话框共用同一套深色设计系统（真实的深色下拉框与复选框，替代浅色系统控件）、分组侧栏带活动项指示条、分段式全局模式控件、双行规则工具栏。v0.15.0 加入规则导入预览：写入前按完整规则身份（进程 + 域名/IP/端口/协议 + 模式）把每条导入规则分类为新增 / 已存在跳过 / 文件内重复跳过；同一进程不同约束的规则会作为新规则导入，而不是被静默丢弃。每个发布包仍内嵌 v0.9.0 引入的未签名 `provenance.json` 构建溯源清单；代码签名本身仍需要证书。
...
1. 从 [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases) 下载 `IntentRoute-AI-v0.16.0-win-x64.zip` 与 `.sha256` 文件。
2. 校验校验和。
