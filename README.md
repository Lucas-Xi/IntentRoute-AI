...
> **Project status: v0.7.0 preview.** IntentRoute AI is useful for testing and early adoption, but it has not yet demonstrated broad production usage. AI output can be incomplete or wrong. Every generated rule is locally validated, added disabled, and must be explicitly enabled by the user.

v0.7.0 completes the localization arc: on top of the window layer, AI-provider error/validator/diagnostic messages and AppService runtime-status text follow the Chinese/English preference (450 resource keys with parity tests). Policy Intelligence finding titles and the persisted default proxy name remain Chinese by design. Downloadable v0.7.0 archives contain all of these changes.
...
1. Download `IntentRoute-AI-v0.7.0-win-x64.zip` and its `.sha256` file from [Releases](https://github.com/Lucas-Xi/IntentRoute-AI/releases).
