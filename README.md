...
> **Project status: v0.6.0 preview.** IntentRoute AI is useful for testing and early adoption, but it has not yet demonstrated broad production usage. AI output can be incomplete or wrong. Every generated rule is locally validated, added disabled, and must be explicitly enabled by the user.

v0.6.0 completes the localization and display stack: every window-layer string — static text, dynamic display values, status messages, and dialogs — follows a Chinese/English preference applied at restart (Chinese by default, stored outside the routing configuration), and the window declares Per-Monitor V2 DPI awareness asserted by the smoke gate. AppService messages and runtime-error text remain Chinese. Downloadable v0.6.0 archives contain all of these changes.
...
1. Download `IntentRoute-AI-v0.6.0-win-x64.zip` and its `.sha256` file from [Releases](https://github.com/Lucas-Xi/IntentRoute-AI/releases).
...
- The UI is Chinese-first: every window-layer string, AI-provider error/validator/diagnostic text, and AppService runtime-status text follow the language preference with English. Policy Intelligence finding titles remain Chinese as stable deterministic-analysis identifiers, as does the persisted default proxy-server name.
