...
Every supported mutation is transactional at the application-state level. A candidate that fails normalization, supported-semantics validation, DPAPI serialization, or atomic filesystem replacement is never published as the active configuration and never queues a runtime apply. Rule and proxy edits preserve a current-session sing-box approval only when the committed executable path is unchanged. Profile replacement, recovery import, and reset clear that approval even when they contain the same path.
